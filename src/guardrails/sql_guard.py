...
def ensure_select_only(sql: str) -> None:
    ...
    if (
        ...
        and not isinstance(parsed, exp.Union)
        and not isinstance(parsed, exp.With)
    ):
...
def enforce_limit(sql: str, default_limit: int, hard_cap: int) -> str:
    try:
        ...
        # Add LIMIT if none
        has_limit = any(
            isinstance(node, exp.Limit) for node in parsed.find_all(exp.Limit)
        )
        if not has_limit:
            logger.info(
                f"No LIMIT found in query. Adding default LIMIT {default_limit}."
            )
        ...
        for node in parsed.find_all(exp.Limit):
            ...
            if isinstance(lit, exp.Literal) and lit.is_number:
                ...
                if val > hard_cap:
                    logger.warning(
                        f"LIMIT {val} exceeds hard cap {hard_cap}. Capping to {hard_cap}."
                    )
...
def check_sensitive_columns(sql: str) -> None:
    lowered = sql.lower()
    hits = [
        sc for sc in SENSITIVE_COLUMNS if f".{sc}" in lowered or f" {sc}" in lowered
    ]
    if hits:
